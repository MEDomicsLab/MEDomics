...
class ModelHandler(Node):
    ...
    def __handle_splitted_data(self, experiment: dict, settings: dict, **kwargs) -> None:
        ...
        try:
            ...
            print("INFO: Falling back to a default list ['Accuracy', 'AUC', 'Recall', 'Prec.', 'F1', 'Kappa']. Adjust as needed.")
        ...
        try:
            ...
            print(f"ERROR: Failed to retrieve models using pycaret_exp.models(). Error: {e}")
            print("INFO: Falling back to a default list ['lr', 'knn', 'rf']. Adjust as needed.")
            models_to_evaluate = ['lr', 'knn', 'rf'] # Fallback list

        split_type = iteration_data['type']
        folds = iteration_data['folds']

        # Iterate through each model to 
        trained_models = []
