...
export default function NetworkCheckModal({ show, onHide, setNetworkChecked, networkChecked }) {
  ...
  const { groupNodeId, updateNode } = useContext(FlowFunctionsContext)
  const { networkClients, setNetworkClients } = useMEDflContext()
  ...
  const onValidateNetwork = async () => {
    setNetworkClients({ ...networkClients, [currentGroupId]: selectedAgentsByGroup[currentGroupId] })
  }
